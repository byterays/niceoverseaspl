...
class PublicController extends BaseController
{
    ...
    public function postApplyJob(ApplyJobRequest $request, int $id = null)
    {
        ...
        $applied = JobApplication::where("phone",$request->input('phone'))->first();
    ...
    public function getCountry(string $slug)
    {      
        ...
        $params = [
            'condition' => [
                'jb_jobs.country_id' => $country->getKey(),
            ],
        ...
        if (request()->ajax()) {
            $view = Theme::getThemeNamespace('views.job-board.partials.country-job-items');
            ...
            return $this
                ->httpResponse()->setData(view($view, compact('jobs', 'country'))->render());
        }

        if (function_exists('admin_bar')) {
            admin_bar()->registerLink(__('Edit this country'), route('country.edit', $country->getKey()), 'country.edit');
        }

        SeoHelper::setTitle($country->name);
       ...
       // Helper::handleViewCount($country, 'viewed_country');

        Theme::breadcrumb()
            ->add(__('Home'), route('public.index'))
            ->add(__('Countries'), JobBoardHelper::getJobCompaniesPageURL())
            ->add($country->name, $country->url);

        do_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, COMPANY_MODULE_SCREEN_NAME, $country);
        ...
        return Theme::scope(
            'job-board.country',
            compact('country', 'data', 'canReview', 'canReviewCompany'),
           // 'Country'//plugins/job-board::themes.country'
        )->render();
    }
