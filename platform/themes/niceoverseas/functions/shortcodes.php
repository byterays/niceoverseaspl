...
app()->booted(function () {
    ...
    Shortcode::register('about-block', __('About Block'), __('About Block'), function (ShortcodeCompiler $shortcode) {
        $tabs = Shortcode::fields()->getTabsData(['item'], $shortcode);
        return Theme::partial("shortcodes.about-block.layout", compact('shortcode', 'tabs'));
    });

    Shortcode::setAdminConfig('about-block', function (array $attributes) {
        return Theme::partial('shortcodes.about-block.admin-config', compact('attributes'));
    });
