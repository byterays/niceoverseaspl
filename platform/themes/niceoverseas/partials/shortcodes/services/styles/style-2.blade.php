        ...
        @if($services->isNotEmpty())
            <div class="row mt-10">
                @foreach($services as $service)                  
                    <div class="col-lg-6 wow animate__animated animate__fadeIn mb-2" data-wow-delay=".{{ $loop->even ? 0 : 2 }}s">
                        <div class="card-offer card-we-do hover-up">
                            @if ($logo = $service->category->getMetadata('icon', true))
                                <div class="card-image mb-5">
                                    <img style='width:100%' src="{{ RvMedia::getImageUrl($service->image) }}" alt="{{ $service->name }}" />
                                </div>
                            @endif
                            <div class="card-info">
                                ...
                                <div class="box-button-offer">
                                    <a href="{{ $service->url }}" class="btn btn-warning font-sm-bold ">{{ __('View Details') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
